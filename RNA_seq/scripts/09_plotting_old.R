...
ATAC_path <- "/oak/stanford/groups/wjg/zshipony/EZH2_scATAC/ArchR_analysis_scg4/Tonsils_new_cluster1"
...
pdf(paste0(save_dir, "/allSamples_RNA_ADT_single/naive_markrs.pdf"),
	height = 8, width = 10)

plot_list_short <- plot_list2[1:3]

ggarrange(plotlist = plot_list_short, ncol = 2, nrow = 2)

dev.off()

pdf(paste0(save_dir, "/allSamples_RNA_ADT_single/gc_markers.pdf"),
	height = 8, width = 10)

plot_list_short <- plot_list2[3:5]

ggarrange(plotlist = plot_list_short, ncol = 2, nrow = 2)

dev.off()

pdf(paste0(save_dir, "/allSamples_RNA_ADT_single/plasmablast_markers.pdf"),
	height = 8, width = 10)

plot_list_short <- plot_list2[5:7]

ggarrange(plotlist = plot_list_short, ncol = 2, nrow = 2)

dev.off()

pdf(paste0(save_dir, "/allSamples_RNA_ADT_single/tcell_markers.pdf"),
	height = 8, width = 10)

plot_list_short <- plot_list2[8:10]

ggarrange(plotlist = plot_list_short, ncol = 2, nrow = 2)

dev.off()

# UMAP of all samples colored by ADT gene
...
pdf(paste0(save_dir, "/allSamples_RNA_ADT_gene_expression_UMAP.pdf"))

adt_rna_list <- c("IGHM", "IGHD", "CD44", "MME", "MS4A1",  "CD38",  "CD27",
	"CD3G", "CD8A", "CD4")
#pal <- wes_palette_kw("Zissou1", 21, type = "continuous")
# pal <- wes_palette_kw("Rushmore1", 21, type = "continuous")

# pal <- grDevices::colorRampPalette(RColorBrewer::brewer.pal(9, "PuRd"))(21)

# plots <- plotDimRed(seurat_object_all, col_by = adt_rna_list, ggrastr = TRUE,
# 	size = 0.5, wesanderson = TRUE, color = pal)

plots <- plotDimRed(seurat_object_all, col_by = adt_rna_list, ggrastr = TRUE,
  size = 0.5, color = c("#00AFBB", "#FFF606"))
...
# Dotplot of RNA and ADT?

adt_list_all <- c("IGHM", "IgM-", "IGHD", "IgD-", "CD44", "CD44-",
                  "MME", "CD10-", "MS4A1", "CD20-", "CD38", "CD38-",
                  "CD27", "CD27-", "CD3G", "CD3-", "CD8A", "CD8a-",
                  "CD4", "CD4-")
...
# This is from seurat DotPlot
object <- seurat_object_all
assay <- NULL
features <- adt_list
...
data.features.rna <- FetchData(object = object, vars = adt_rna_list)
data.features.adt <- FetchData(object = object, vars = adt_list_fetch)
data.features.rna$id <- object[[group.by, drop = TRUE]]
data.features.adt$id <- object[[group.by, drop = TRUE]]

id.levels <- levels(x = data.features.rna$id)
data.features.rna$id <- as.vector(x = data.features.rna$id)

data.features.adt$id <- as.vector(x = data.features.adt$id)
# if (!is.null(x = split.by)) {
...
# }
data.plot.rna <- lapply(
    X = unique(x = data.features.rna$id),
    FUN = function(ident) {
      data.use <- data.features.rna[data.features.rna$id == ident, 1:(ncol(x = data.features.rna) - 1), drop = FALSE]
      avg.exp <- apply(
        X = data.use,
        MARGIN = 2,
        FUN = function(x) {
...
data.plot.adt <- lapply(
    X = unique(x = data.features.adt$id),
    FUN = function(ident) {
      data.use <- data.features.adt[data.features.adt$id == ident, 1:(ncol(x = data.features.adt) - 1), drop = FALSE]
      avg.exp <- apply(
        X = data.use,
        MARGIN = 2,
        FUN = function(x) {
...
names(x = data.plot.rna) <- unique(x = data.features.rna$id)
names(x = data.plot.adt) <- unique(x = data.features.adt$id)
data.plot.rna <- lapply(
    X = names(x = data.plot.rna),
    FUN = function(x) {
      data.use <- as.data.frame(x = data.plot.rna[[x]])
...
data.plot.adt <- lapply(
    X = names(x = data.plot.adt),
    FUN = function(x) {
      data.use <- as.data.frame(x = data.plot.adt[[x]])
      data.use$features.plot <- rownames(x = data.use)
      data.use$id <- x
      return(data.use)
    }
)
data.plot.rna <- do.call(what = 'rbind', args = data.plot.rna)
if (!is.null(x = id.levels)) {
    data.plot.rna$id <- factor(x = data.plot.rna$id, levels = id.levels)
}
data.plot.adt <- do.call(what = 'rbind', args = data.plot.adt)
if (!is.null(x = id.levels)) {
    data.plot.adt$id <- factor(x = data.plot.adt$id, levels = id.levels)
}

avg.exp.scaled.rna <- sapply(
    X = unique(x = data.plot.rna$features.plot),
    FUN = function(x) {
      data.use <- data.plot.rna[data.plot.rna$features.plot == x, 'avg.exp']
      data.use <- scale(x = data.use)
      data.use <- MinMax(data = data.use, min = col.min, max = col.max)
      return(data.use)
    }
)
avg.exp.scaled.adt <- sapply(
    X = unique(x = data.plot.adt$features.plot),
    FUN = function(x) {
      data.use <- data.plot.adt[data.plot.adt$features.plot == x, 'avg.exp']
      data.use <- scale(x = data.use)
      data.use <- MinMax(data = data.use, min = col.min, max = col.max)
      return(data.use)
    }
)
avg.exp.scaled.rna <- as.vector(x = t(x = avg.exp.scaled.rna))
avg.exp.scaled.adt <- as.vector(x = t(x = avg.exp.scaled.adt))
# if (!is.null(x = split.by)) {
#     avg.exp.scaled <- as.numeric(x = cut(x = avg.exp.scaled, breaks = 20))
# }

# avg.exp.scaled.numeric.rna <- as.numeric(x = cut(x = avg.exp.scaled.rna, breaks = 20))
# avg.exp.scaled.numeric.adt <- as.numeric(x = cut(x = avg.exp.scaled.adt, breaks = 20))



data.plot.rna$avg.exp.scaled <- avg.exp.scaled.rna
data.plot.rna$features.plot <- factor(
    x = data.plot.rna$features.plot,
    levels = rev(x = adt_rna_list)
)
data.plot.adt$avg.exp.scaled <- avg.exp.scaled.adt
data.plot.adt$features.plot <- factor(
    x = data.plot.adt$features.plot,
    levels = rev(x = adt_list_fetch)
)

data.plot.rna$pct.exp[data.plot.rna$pct.exp < dot.min] <- NA
data.plot.rna$pct.exp <- data.plot.rna$pct.exp * 100
data.plot.adt$pct.exp[data.plot.adt$pct.exp < dot.min] <- NA
data.plot.adt$pct.exp <- data.plot.adt$pct.exp * 100

# Use this to get colors for each

# data.plot.rna$colors <- mapply(
#   FUN = function(value) {
#     return(colorRampPalette(colors = c('grey', 'blue'))(20)[value])
#   },
#   value = avg.exp.scaled
# )

# data.plot.adt$colors <- mapply(
#   FUN = function(value) {
#     return(colorRampPalette(colors = c('grey', 'red'))(20)[value])
#   },
#   value = avg.exp.scaled
# )

# color_function <- function(data_plot, color){
#   value <- data_plot[[5]]
#   return(colorRampPalette(colors = c("grey", color))(20)[value])
# }

# data.plot.rna$colors <- apply(data.plot.rna, 1, color_function, "blue")

data.plot.rna$type <- "RNA"
data.plot.adt$type <- "ADT"

data.plot <- rbind(data.plot.rna, data.plot.adt)
...
adt_list_all <- c("IGHM", "adt_IgM-", "IGHD", "adt_IgD-", "CD44", "adt_CD44-",
                  "MME", "adt_CD10-", "MS4A1", "adt_CD20-", "CD38", "adt_CD38-",
                  "CD27", "adt_CD27-", "CD3G", "adt_CD3-", "CD8A", "adt_CD8a-",
                  "CD4", "adt_CD4-")

data.plot$features.plot <- factor(
    x = data.plot$features.plot,
    levels = rev(x = adt_list_all)
)
...
data.plot.rna$id <- factor(
    x = data.plot.rna$id,
    levels = c("Naive_B", "Activated_B", "Light_GC_B", "Dark_GC_B",
               "Plasma_B", "CD4_T", "CD8_T", "Monocyte_DC", "Dendritic")
)

data.plot.adt$id <- factor(
    x = data.plot.adt$id,
    levels = c("Naive_B", "Activated_B", "Light_GC_B", "Dark_GC_B",
               "Plasma_B", "CD4_T", "CD8_T", "Monocyte_DC", "Dendritic")
)
...
data.plot.adt2 <- data.plot.adt

data.plot.adt2$pct.exp <- 0

data.plot.rna.2 <- rbind(data.plot.rna, data.plot.adt2)

pdf(paste0(save_dir, "/allSamples_RNA_ADT_gene_expression_dotplot.pdf"))


plot <- ggplot(data = data.plot, mapping = aes_string(x = 'features.plot', y = 'id')) +
    geom_point(data = data.plot.rna.2, mapping = aes_string(size = 'pct.exp', color = 'avg.exp.scaled')) +
    scale_colour_gradientn(colours = c("grey", "blue")) +
    ggnewscale::new_scale_colour() +
    geom_point(data = data.plot.adt, mapping = aes_string(size = 'pct.exp', color = 'avg.exp.scaled')) +
  ...
  # }
plot + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90)) + coord_flip()
...
txn_regulators <- c("BCL11A","MEF2A", "MEF2B", "MEF2C","PAX5", "POU2F2", "CTCFL","CREB1",
	                "EBF1", "LMO2", "CTCF", "HMGA1", "TCF3",
	                "ELF1","IRF4", "EOMES", 
	                "SMAD9", "CEBPD", "CEBPB", "SPI1", "SPIB", "STAT1",
	                "IRF1", "LEF1")


# Figure 3
# Markers from Archr on a dotplot
pdf(paste0(save_dir, "allSamples_archr_markers_dotplot.pdf"),
	height = 10, width = 12)

plot1 <- DotPlot(object = seurat_object_all, features = txn_regulators,
    group.by = "cell_type",
    cols = c("lightgrey", "blue"))
...
##############################
# Plots for GC B cells (RNA) #
##############################

seurat_gc <- readRDS(seurat_path_gc)

pdf(paste0(save_dir, "allSamples_GCBcells_RNA_UMAP.pdf"),
	height = 12, width = 10)

plotDimRed(seurat_gc, col_by = "cell_type", color = colors_cell_type_new,
	ggrastr = TRUE, size = 0.5)

dev.off()

# UMAP of all samples, colored by cluster
pdf(paste0(save_dir, "allSamples_GCBcells_RNA_UMAP_cluster.pdf"),
	height = 12, width = 12)

plotDimRed(seurat_gc, col_by = "new_cluster", ggrastr = TRUE,
	size = 0.5)

dev.off()

marker_list <- c("POU2F2", "MYC", "CD83", "CD40", "NFKBIA", "EGR1", "EGR2", "EGR3",
	"SLAMF1", "ICAM1", "IL2RG", "BATF", "TCF3", "HMMR",
	"FOXP1", "CXCR4", "PCNA", "TOP2A", "MKI67")

pdf(paste0(save_dir, "GCBcells_RNA_literature_markers_dotplot.pdf"),
	height = 10, width = 8)

plot1 <- DotPlot(object = seurat_gc, features = marker_list,
...
save_dir_gcbcell <- "output/allSamples_GCBcells_snakemake/"


plotDimRed(seurat_gc, col_by = all_markers,
	save_plot = paste0(save_dir_gcbcell, "images/UMAP_markers.pdf"))


motif_enrichment <- c("ZBTB7A", "CTCFL", "KLF6", "SP1", "EGR1", "SP2",
	"ZFX", "KLF7", "KLF5", "SMAD5", "DNMT1")

pdf(paste0(save_dir, "GCBcells_enriched motifs.pdf"),
	height = 10, width = 8)

plot1 <- DotPlot(object = seurat_gc, features = motif_enrichment,
    group.by = "new_cluster",
    cols = c("lightgrey", "blue"))
plot1 + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90))
...
plotDimRed(seurat_gc, col_by = motif_enrichment, size = 1,
	save_plot = paste0(save_dir, "GCBcells_enriched_motifs.pdf"))


#######################
# plots for new GC

seurat_gc_new <- readRDS(seurat_path_gc_new)


rna_plot_list <- c("IGHM", "IGHD", "CD44", "MME", "MS4A1",  "CD38",  "CD27",
  "CD3G", "CD8A", "CD4", "CXCR4", 
  "POU2F2", "MYC", "CD83", "CD40", "NFKBIA", "EGR1", "EGR2", "EGR3",
  "SLAMF1", "ICAM1", "IL2RG", "BATF", "TCF3", "HMMR",
  "FOXP1", "PCNA", "TOP2A", "MKI67", "FCRL4",
...
# Run magic
seurat_gc_new <- magic(seurat_gc_new, genes = rna_plot_list)


pdf(paste0(save_dir, "allSamples_GC_seed17_gene_imputed.pdf"))
DefaultAssay(seurat_gc_new) <- "MAGIC_SCT"
pal <- wes_palette_kw("Zissou1", 21, type = "continuous")
plots <- plotDimRed(seurat_gc_new, col_by = rna_plot_list, ggrastr = TRUE,
  wesanderson = TRUE, color = pal)
plots
# plots <- lapply(1:length(plots), function(x){
#   plot <- plots[[x]]
...
# Run magic on ADTs
DefaultAssay(seurat_gc_new) <- "ADT"
seurat_object_all <- magic(seurat_gc_new)


pdf(paste0(save_dir, "allSamples_GC_seed17_ADT_imputed.pdf"))
DefaultAssay(seurat_gc_new) <- "MAGIC_ADT"
#pal <- wes_palette_kw("Zissou1", 21, type = "continuous")
plots <- plotDimRed(seurat_gc_new, col_by = rownames(seurat_gc_new),
  ggrastr = TRUE)
#plots
# plots <- lapply(1:length(plots), function(x){
#   plot <- plots[[x]]
...
#####################################
# Plots for nonLymphoid cells (RNA) #
#####################################

seurat_nonLympyoid <- readRDS(seurat_path_nonLymphoid)

pdf(paste0(save_dir, "allSamples_nonLymphoid_RNA_UMAP.pdf"),
	height = 12, width = 10)

plotDimRed(seurat_nonLympyoid, col_by = "cell_type", color = colors_cell_type_new,
	ggrastr = TRUE, size = 1)

dev.off()

pdf(paste0(save_dir, "allSamples_nonLymphoid_RNA_UMAP_cluster.pdf"),
	height = 12, width = 12)

plotDimRed(seurat_nonLympyoid, col_by = "cluster", ggrastr = TRUE,
	size = 1)
...
TCell_markers <- FindMarkers(seurat_TCells, ident.1 = 0, ident.2 = 5, slot = "scale.data")

write.csv(TCell_markers, paste0(t_cell_dir, "/files/0v5_markers.csv"))
...
names(colors_cluster_ATAC) <- 0:(nclusters_ATAC - 1)
sample_data$Clusters2 <- factor(sample_data$Clusters2,
    levels = names(cell_groups))
...
one_barplot_ATAC(ATAC_meta = sample_data, subsample_list = tonsil_list,
  sample_name = "allSamples", subsample_by = "Tonsil", meta_data_col = "Clusters2",
