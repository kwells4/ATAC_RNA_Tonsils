...
# Remove TEAD1, NFIB, and NFIA because they don't have much expression in RNA
...
genes_plot <- lapply(txn_regulators, function(x){
  plot_data_genes <- plot_data[plot_data$features.plot == x,]
...
genes_plot <- unlist(genes_plot)

txn_regulators_filtered <- txn_regulators[genes_plot]

# Order genes based on in what cell type they are most highly expressed
ordering <- lapply(txn_regulators_filtered, function(x){
  plot_data_genes <- plot_data[plot_data$features.plot == x, ]
  return(plot_data_genes[plot_data_genes$pct.exp == max(plot_data_genes$pct.exp), ]$id)
  })

ordering <- unlist(ordering)

ordering <- as.character(ordering)

names(ordering) <- txn_regulators_filtered

# Make the order of genes the same as the order of cell type in the plot
ordering <- factor(ordering, levels = levels(seurat_object_all$cell_type))
ordering <- ordering[order(ordering)]

pdf(paste0(save_dir, "allSamples_archr_markers_dotplot.pdf"),
	height = 10, width = 12)

plot1 <- DotPlot(object = seurat_object_all, features = names(ordering),
    group.by = "cell_type",
    cols = c("lightgrey", "blue"))
...
# Same plot based on the higher resolution cell type
...
# Make the order of genes the same as the order of cell type in the plot
ordering <- factor(ordering, levels = levels(seurat_object_all$cell_type_high_res))
ordering <- ordering[order(ordering)]

pdf(paste0(save_dir, "allSamples_highResCellType_archr_markers_dotplot.pdf"),
  height = 10, width = 12)
...
keep <- lapply(seq_len(ncol(mat)), function(x) {
    idx <- head(order(mat[, x], decreasing = TRUE), 50)
    rownames(mat)[idx[which(mat[idx, x] > 20)]]
}) %>% unlist %>% unique
...
# if (pMax != 100) {
#     rownames(mat[[1]]) <- paste0(rownames(mat[[1]]), " (", 
#         round(mat$max), ")")
#     rownames(passMat) <- rownames(mat[[1]])
# }

write.csv(mat, paste0(ATAC_path, "/files/enriched_motifs.csv"))

heatmapEM <- plotEnrichHeatmap(enrichMotifs, n = 5, transpose = TRUE)

plotPDF(heatmapEM, name = "Motifs-Enriched-Marker-Heatmap-5", width = 12,
