...
#input_loom = "output/allSamples/files/allSamples_RNA_velo.loom"
pseudotime_output = snakemake.output[3]

cluster_png = snakemake.output[0]
cell_type_png = snakemake.output[1]
subgroup=snamekame.params[1]
...
adata.obs["seurat_cluster"] = seurat_clust_ord["new_cluster"]
...
if subgroup == "GCBcells":
	plt.savefig(save_dir + "images/GCBcells_rna_velocity_confidence.pdf")
	...
	plt.savefig(save_dir + "images/BCells_rna_velocity_confidence.pdf")
	adata.uns["cell_type_colors"] = ("#377EB8", "#984EA3", "#4DAF4A", "#E41A1C", "#FF7F00")
	adata.uns["seurat_cluster_colors"] = ("#E41A1C", "#3A85A8", "#629363",
		"#C4625D", "#FFC81D", "#BF862B", "#EB7AA9", "#999999")
...
if subgroup == "GCBcells":
	...
	# The default was backwards, so I took the min and max pseudotime index
	# And reversed them.
	scv.tl.velocity_pseudotime(adata)
	adata.obs["row_num"] = np.arange(len(adata.obs))
	min_loc = adata.obs[adata.obs.velocity_pseudotime == adata.obs.velocity_pseudotime.min()].row_num
	max_loc = adata.obs[adata.obs.velocity_pseudotime == adata.obs.velocity_pseudotime.max()].row_num
	min_loc = int(min_loc)
	max_loc = int(max_loc)
	scv.tl.velocity_pseudotime(adata, root = max_loc, end = min_loc)
