...
subset_cells <- subset_list[[output_name]]
...
size_list <- c(BCells = 0.5, GCBcells = 0.5, ActiveNaiveB = 0.5, TCells = 0.5,
  nonLymphoid = 0.5, NonGCBcells = 0.5)
ndims_list <- c(BCells = 30, GCBcells = 30, ActiveNaiveB = 25, TCells = 20,
...
seurat_subset <- run_harmony(seurat_object_merge = seurat_subset,
  ...
  plot_ADT = TRUE, ndims = ndims_list[[output_name]],
  variable_features = variable_features_subset)
...
plotDimRed(sample_object = seurat_subset, size = size_list[[output_name]],
...
de_genes <- cell_markers(seurat_subset,
  sample_name = paste0("all_samples_", output_name),
  save_dir = save_dir, sctransform = TRUE)
